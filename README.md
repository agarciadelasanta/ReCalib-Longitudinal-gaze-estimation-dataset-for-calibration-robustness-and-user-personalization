...
* **Output**: 
   ...
   * **Image overlay**: visualization of different elements such as gace vector, 2D landmarks elements over the original image.
...
* **Input**: Requires the flattened dataset index (`dataset_summary.csv`). Optionally, `id_user` and `id_session` can be configured within the script to filter the analysis to a specific participant or session.
* **Output**: Displays the generated figures interactively and exports them as `.png` images to a designated `temps/` directory, along with a filtered `.csv` file.


Below some of the many possible outputs generated by `gaze_visualization_csv.py`:
